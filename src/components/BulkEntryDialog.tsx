...
export function BulkEntryDialog({ open, onOpenChange, products }: BulkEntryDialogProps) {
    ...
    setLines((prev) =>
      prev.map((l) => {
        ...
        if (field === "productId") {
          ...
          updated.unitCost = prod ? String(prod.last_unit_cost ?? prod.average_cost ?? 0) : "";
        }
        return updated;
      })
    );
  ...
  const needsReason = isBackdating;
  const isValid =
    ...
    mutationFn: async () => {
      ...
      for (const line of validLines) {
        ...
        const insertData: any = {
          ...
          notes: notes || "Entrada masiva",
          restaurant_id: restaurantId,
          source_module: "bulk_entry",
        };
    ...
    >
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          {/* Date picker */}
          <Card className="border-dashed border-emerald-500">
              ...
              {lines.map((line, idx) => {
                ...
                return (
                  <div key={line.id} className="grid grid-cols-12 gap-2 items-end">
                    ...
                    <div className="col-span-2">
                      {idx === 0 && <Label className="text-xs text-muted-foreground">Unidad</Label>}
                      {prod ? (
                        <UnitSelector
                    ...
                    <div className="col-span-1 text-xs text-muted-foreground flex items-center h-10">
                      {lineCost > 0 && <span>${lineCost.toLocaleString("es-CO", { maximumFractionDigits: 0 })}</span>}
                    </div>
            ...
            <KioskTextInput
              ...
              placeholder={
                needsReason
                  ...
                  : "Observaciones de las entradas..."
              }
              keyboardLabel="Notas de entrada"
            ...
            <Card>
              <CardContent className="pt-4 space-y-1">
                ...
                <div className="flex justify-between text-sm font-semibold">
                  ...
                  <span>${totalCost.toLocaleString("es-CO", { minimumFractionDigits: 2 })}</span>
                </div>
          ...
          >
            {submitBulk.isPending ? (
              ...
              <>
                ...
                Confirmar {validLines.length} Entrada{validLines.length !== 1 ? "s" : ""}
              </>
            )}
          </Button>
        </div>
