...
const emptyForm: RoomTypeForm = { name: "", description: "", base_rate: 0, max_occupancy: 2, active: true };
...
export default function RoomTypesTab() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = { restaurant_id: restaurantId, name: form.name.trim(), description: form.description.trim(), base_rate: form.base_rate, max_occupancy: form.max_occupancy, active: form.active };
      if (editId) {
  ...
  const openEdit = (t: any) => { setEditId(t.id); setForm({ name: t.name, description: t.description || "", base_rate: t.base_rate, max_occupancy: t.max_occupancy, active: t.active }); setOpen(true); };
  ...
  return (
    <div className="space-y-4">
      ...
      <Table>
        <TableHeader>
          <TableRow><TableHead>Nombre</TableHead><TableHead>Tarifa Base</TableHead><TableHead>Capacidad</TableHead><TableHead>Estado</TableHead><TableHead className="w-24">Acciones</TableHead></TableRow>
        </TableHeader>
        <TableBody>
          {isLoading ? <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">Cargando...</TableCell></TableRow> :
           types?.length === 0 ? <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">No hay tipos registrados</TableCell></TableRow> :
           types?.map((t: any) => (
            <TableRow key={t.id}>
              <TableCell className="font-medium">{t.name}</TableCell>
              <TableCell>${t.base_rate?.toLocaleString()}</TableCell>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label>Tarifa Base</Label><Input type="number" value={form.base_rate} onChange={e => setForm({ ...form, base_rate: +e.target.value })} /></div>
              <div><Label>Capacidad Máx.</Label><Input type="number" value={form.max_occupancy} onChange={e => setForm({ ...form, max_occupancy: +e.target.value })} /></div>
            </div>
